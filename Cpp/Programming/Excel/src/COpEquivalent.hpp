 ...
 * */
class COpEquivalent : public CExpression {
    public:
        /**
          * Constructor for COpEquivalent which inherits from CExpression class
          ...
          * */
        COpEquivalent(const std::map<CPos, std::pair<ASTnode, std::vector<CPos>>> &data, ASTnode lhs, ASTnode rhs)
                : CExpression(data), m_Lhs(std::move(lhs)), m_Rhs(std::move(rhs)) {}

        /**
         * Performs == operation on its 2 children
         * @returns 1 if equal, 0 if not equal but with same time, otherwise std::monostate
         * */
        ...
        /**
         * Creates a deep copy of COpEquivalent instance
         * @param[in] map - new map to which new instance will point to
         * @returns a new std::unique_ptr to COpEquivalent
         * */
