...
public class OwnerServiceTest {
    ...
    public void testUpdateOwner() {
        Owner oldOwner = new Owner("NewOner@mail.com", null, List.of());
        Owner expectedNewOwner = new Owner("NewOner@mail.com", "123123123", List.of());
        when(ownerRepository.existsById(expectedNewOwner.getEmail())).thenReturn(true);
        when(ownerRepository.getReferenceById(oldOwner.getEmail())).thenReturn(oldOwner);
        when(ownerRepository.save(any(Owner.class))).thenReturn(expectedNewOwner);
        OwnerService ownerService = new OwnerServiceImpl(ownerRepository);
